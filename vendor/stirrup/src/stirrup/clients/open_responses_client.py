...
def _to_open_responses_tools(tools: dict[str, Tool]) -> list[dict[str, Any]]:
    ...
    for t in tools.values():
        ...
        if t.raw_schema is not None:
            tool_def["parameters"] = t.raw_schema
        elif t.parameters is not EmptyParams:
...
class OpenResponsesClient(LLMClient):
    ...
    async def generate(
        ...
        # Include summary="concise" so reasoning summaries are returned even with tool calls.
        # Note: "auto"/"detailed" return empty summaries when tool calls are present on
        # gpt-5.4/5.5, but "concise" works reliably across both tool and non-tool turns.
        if self._reasoning_effort:
            request_kwargs["reasoning"] = {"effort": self._reasoning_effort, "summary": "concise"}
