...
class ChatCompletionsClient(LLMClient):
    ...
    def __init__(
        ...
        *,
        ...
        cap_max_completion_tokens: bool = False,
        skip_reasoning_with_tools: bool = False,
    ) -> None:
        ...
        Args:
            ...
            cap_max_completion_tokens: If True, cap max_completion_tokens at
                min(max_tokens, 16384) to avoid API rejection. Defaults to False.
            skip_reasoning_with_tools: If True, skip reasoning_effort when tools
                are present (OpenAI rejects reasoning_effort + function tools).
                Defaults to False.
        """
        self._model = model
        ...
        self._cap_max_completion_tokens = cap_max_completion_tokens
        self._skip_reasoning_with_tools = skip_reasoning_with_tools
    ...
    async def generate(
        ...
        # Use max_completion_tokens from kwargs if provided, else use max_tokens
        # but cap at a reasonable output limit to avoid API rejection
        if "max_completion_tokens" not in self._kwargs and "max_tokens" not in self._kwargs:
            if self._cap_max_completion_tokens:
                request_kwargs["max_completion_tokens"] = min(self._max_tokens, 16384)
            else:
        ...
        # Skip when tools are present if skip_reasoning_with_tools is set
        # (OpenAI rejects reasoning_effort + function tools)
        if self._reasoning_effort and not (self._skip_reasoning_with_tools and tools):
        ...
        if hasattr(msg, "reasoning_content") and msg.reasoning_content:
            ...
            r = msg.model_extra.get("reasoning") or msg.model_extra.get("reasoning_content")
            if r:
                reasoning = Reasoning(content=r)
